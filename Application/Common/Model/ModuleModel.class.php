...
//         exit;
        ...
        foreach ($modules as $m) {
            $name = ucfirst($name);
            if (isset($m['is_setup']) && $m['is_setup'] == 0 && $m['name'] == $name) {
                header("Content-Type: text/html; charset=utf-8");
                exit('您所访问的模块未安装，禁止访问。');
            }
            
			$account_group = session('account_group');
            if(!empty($account_group) && !in_array($name,$this->getAuthModelName())) {
                $this->noAuthAction();
            }
        }
    ...
    public function getAuthModelName() {
        switch(session('account_group')) {
            case 5:
                $modelNameArr = array('Home','Corp','Test');
                break;
            default:
                $modelNameArr = array('Home','Corp','Test');
                break;
     ...
     * 授权跳转
     */
    private function noAuthAction() {
        switch(session('account_group')) {
            case 5:
                $url = U('Corp/index/index');
                redirect($url);
                break;
            default:
                $url = U('Corp/index/index');
