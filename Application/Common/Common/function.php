...
use Api\Controller\OssController;
use Think\Model;
...
function is_need()
{
	...
	if (empty($user)) {
		...
		return session('user_auth_sign') == data_auth_sign($user) ? $user['need_uid'] : 0;
	}
}
 ...
 * 检测用户是否有该权限
 */
function is_auth($auth,$val){
	if(is_need() > 0){
		return $auth&$val == 0 ? 0 : $auth&$val;
	}else{
		return $val;
	}
}
...
{
	$prefix = C('DB_PREFIX');//SQL语句不支持数组，这里改变为变量
	$tstr = date('Ymd');
	$tableName = 'action_log_'.$tstr;
	
	$Model = new Model();
	$tables = $Model->db()->getTables();
	
	//创建log表
	if(!in_array($prefix.$tableName,$tables)){
		//不存在当天日志表时，需要即时创建
		$sql=<<<SQL
...
SQL;
		$return = $Model->execute($sql);unset($sql);
		
		if($return === false) return '创建表失败';
	}
	
	//创建关联表
	$reTableName = 'action_consensus_'.$tstr;
	if(!in_array(C('DB_PREFIX').$reTableName,$tables)){
		$sql=<<<SQL
CREATE TABLE IF NOT EXISTS `{$prefix}{$reTableName}` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `log_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '日志id',
  `sConsensusName` varchar(255) NOT NULL DEFAULT '' COMMENT '生成报告的报告名称',
  `sGroupName` varchar(255) NOT NULL DEFAULT '' COMMENT '生成报告的组名',
  `iAddTime` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '生成报告的日期',
  `sRemarkJson` text NOT NULL DEFAULT '' COMMENT '备注json数据',
  `iType` tinyint(2) NOT NULL DEFAULT '1' COMMENT '报告类型 默认1',
  `iBeginTime` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '生成报告时选择的开始时间',
  `iEndTime` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '生成报告时选择的结束时间',
  PRIMARY KEY (`id`),
  KEY `log_id_ix` (`log_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='报告关联日志数据表' AUTO_INCREMENT=1;
SQL;
		$return = $Model->execute($sql);unset($sql);
		
		if($return === false) return '创建表失败';
	}
	//var_dump($tables,1);exit;
    ...
    return array('log_id'=>$log_id,'sTime'=>$tstr);
}


/**
 * 加入日志关联数据-报告数据表
 * @param unknown $log_id 日志ID
 * @param unknown $sTime 要插入表的后缀
 * @param unknown $data 要插入的数据
 */
function action_consensus($log_id, $sTime, $data)
{
	$tableName = 'action_consensus_'.$sTime;
	
	$data = M($tableName)->create($data);
	if($data){
		$data['log_id'] = $log_id;
		$data['iAddtime'] = time();
		$res = M($tableName)->add($data);
		
		if(!$res) return '数据插入失败';
	}else{
		return '数据错误';
	}
}
 ...
 * @author colin
 */
function multDownloadFile($url,$filePath) {
    static $count = 0;
    static $urls = array();
    ...
    if(!empty($url)) {
        ...
        $path = preg_replace('/http[s]*:\/\/[\s\S]*?\//','', dirname($url));
        $savePath = './Public/file/'.$filePath.'/'.$path;
        $fileName = basename($url);
        $info['fileName'] = $fileName;
        $info['savePath'] = $savePath;

        if($count<=10) {
            return $info;//有10个附件时才进行下载
        }
 ...
 */
function send_email_for_report($to = '', $subject = '', $body = '', $name = '', $attachment = null, $type=1, $mail_smtp_user = 'MAIL_SMTP_USER') {
    $commonModel = D('Api/Common');
    $code = '';
    $code .= $data = array();
    $code .= $data['sTo'] = $to;
    $code .= $data['sSubject'] = $subject;
    $code .= $data['sName'] = $name;
    $code .= $data['sBody'] = $body;
    $code .= $data['attachment'] = serialize($attachment);
    $code .= $data['tiType'] = (int)$type;
    $code .= $data['sSmtpUser'] = $mail_smtp_user;
    $data['iAddTime'] = time();
    
    $code .= date('Y-m-d H:i');
    $data['sTrCode'] = msubstr(md5($code),0,16,'utf-8',false);
    
    $commonModel->dataAdd('email_box',$data,array('sTrCode'=>$data['sTrCode']));
}
...
/**
 * 对传递过来的URL进行处理
 * @param URL $url 要处理的URL
 * @param IP $ip URL前要加的参数
 */
 ...
 * @param IP $ip URL前要加的参数
 */
function IpU($url='',$vars='',$suffix=true,$domain=false) {
    
    $url = IpStatic(U($url,$vars,$suffix,$domain));
 ...
 * 根据传递的id，获得文件的本地存放路径
 * @param  int $id 文件id
 * @return url     返回返回拼接后的url
 */
function getFileUrl($id) {
    $map['id'] = $id;
    $result = M('File')->where($map)->find();

    $url = IpStatic($result['savepath'].'/'.$result['name']);
    return $url;
}
 ...
 * 实现Oss文件URL签名访问
 */
function Signatureurl($id){
    $info = getInfo($id);
    $OssController = new OssController();
    $signUrl = $OssController->Signatureurl($info,'');
    return $signUrl;
}
...
function getUrl($url){
    ...
    $OssController = new OssController();
    $signUrl = $OssController->Signatureurl('',$url);

    return $signUrl;
}
//oss--链接转化为oss范文api链接地址
function getOssUrlByFullUrl($url){
    
    //临时url兼容处理--20160603
    $url = preg_replace('/\/{2,}/i', '/', $url);
    
    $OssController = new OssController();
    $signUrl = $OssController->SignatureFullurl($url);
 ...
 */
function fileExistFromOss($object) {
    $OssModel = new OssController();
    try {
        $result = $OssModel->fileExistFromOss($object);
        return $result;
    } catch (Exception $e) {
        return false;
    }
