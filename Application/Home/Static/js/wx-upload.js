...
(function(window, document, $, wx, undefined){
    ...
    function init($elem) {
        var options = {
			...
			progress   : typeof $elem.attr(prefix+"-progress") != "undefined",
            isUseFlash : typeof $elem.attr(prefix+"-flash") != "undefined",
            errorcb    : $elem.attr(prefix+"-errorcb") || undefined
        };
        ...
        options.errorcb = $.isFunction(window[options.errorcb]) ? window[options.errorcb] : null;
    ...
    function complete(responseText, options, $input) {
        try{
            ...
            if(data[wx.config.dataFlag] == wx.config.dataSuccessVal){
                ...
                var tip_title = '系统提示';
                var tip_yes = '确定';
                wx.alert(data[wx.config.dataInfo],function(){},{tip_title:tip_title,tip_yes:tip_yes,tip_width:'376px',attachBg: 1,height: 650});
            }
        }
    ...
    function h5() {
        ...
        for(var i=0; i<files.length; i++){
            ...
            if(options.size && files[i].size > options.size){
                if(options.errorcb == null) {
					if(parseInt(files[i].size) > 10485760){
						wx.alert(ThinkPHP.LANG.PIC_BIG_10,function(){},{tip_title:tip_title,tip_width:'376px',tip_yes:tip_yes,attachBg: 1,height: 650});
					}else{
						wx.alert(ThinkPHP.LANG.PIC_BIG,function(){},{tip_title:tip_title,tip_width:'376px',tip_yes:tip_yes,attachBg: 1,height: 650});
					}
                }else{
                    options.errorcb(ThinkPHP.LANG.PIC_BIG)
                }
                ...
                if(options.errorcb == null) {
                    wx.alert(ThinkPHP.LANG.NO_STYLE_ALSO,function(){},{tip_title:tip_title,tip_width:'376px',tip_yes:tip_yes,attachBg: 1,height: 650});
                }else{
                    options.errorcb(ThinkPHP.LANG.NO_STYLE_ALSO)
                }
				$input.unbind("change").val('').change(h5);
                continue;
            }
    ...
    function flash($input) {
        ...
        window.wxUploadFlashError = function(data) {
            ...
            data = $.parseJSON(data);
            if(data.status === 2)
                wx.alert(ThinkPHP.LANG.PIC_BIG,function(){},{tip_title:tip_title,tip_yes:tip_yes,tip_width:'376px',attachBg: 1,height: 650});
            else
                wx.alert(ThinkPHP.LANG.NO_STYLE_ALSO,function(){},{tip_title:tip_title,tip_yes:tip_yes,tip_width:'376px',attachBg: 1,height: 650});
        };
    }
