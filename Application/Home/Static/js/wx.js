...
(function(window, document, $, undefined){
  ...
  */
  ...
  wx.sendData=function(url, options, callback) {
    ...
    //页面token加入
    urlParam += (urlParam==''?
  		  (window.Think.PAGE_TOKEN != undefined?'PAGE_TOKEN='+window.Think.PAGE_TOKEN:'')
  		  :(window.Think.PAGE_TOKEN != undefined?'&PAGE_TOKEN='+window.Think.PAGE_TOKEN:''))
    ...
    ajaxObj = $.ajax({
	  ...
	  jsonp: false,
      //(window.Think.PAGE_TOKEN != undefined?:)
      data:urlParam,
      dataType: options.dataType || "json",
      beforeSend: function(request) {
    	  if(options.header) {
    		  window[options.xhr];
    		  options.header.headers.forEach(function(v) {
    			  request.setRequestHeader(v, window[options.header.name].getResponseHeader(v));
    		  });
    	  }
    	  if(options.disabledGzip) {
          request.setRequestHeader('Accept-Encoding-Disbaled', 1);
    	  }
      },
      success:function(backData, textStatus,xhr) {
        ...
        if(xhr.responseText !== undefined) {
          if(xhr.responseText.indexOf('`reload_page_flag`') != -1) {
              var tip_title = ThinkPHP.LANG.CONFIRM_TIP;
              var tip_yes = ThinkPHP.LANG.GO_LOGIN;
              {
                wx.alert(ThinkPHP.LANG.TIME_OVER,function(){},{tip_title:tip_title,tip_yes:tip_yes,tip_width:"352px",attachBg: 1, height: 650}
                );
              };
              $('.button_alert').attr('onclick','window.open(U("Home/Member/login",["check",1]),"_blank")');
              callback.call(_this,{"wxStatus":"error",message:'login out'});
              return ;
          }
        ...
        try  {  
          ...
          if(xhr.statusText !== 'abort') {
            globalRecordError('STATUS:'+'\\n'+xhr.status+'\\n\\n'+'TYPE:'+'\\n'+xhr.statusText+'\\n\\n'+'URL:'+'\\n'+url+'\\n\\n'+'PARAM:'+'\\n'+urlParam);
          }

          if(callback)
            callback.call(_this,{"wxStatus":"error",message:textStatus,"text":xhr.responseText});
          return;
        }
  ...
  */
  wx.confirm = function(content, callback, opts) {
    ...
    return _pop(wx.tpl(wx.config.confirm,opts),function(){},opts);
  };
  ...
  */
  ...
  //弹框的核心方法
	function _pop(content, callback, opts) {
		...
		opts.documentScrollTop = $(document).scrollTop();
    ...
    $("#wx-Js-pop-body-box").append(temp);
    ...
    function _close(){
      if(opts.attachBg) {
    	  $("#body_box").css({"overflow":"auto","position":"static","height":"auto"});
    	  $("#wx-Js-pop-body-box").removeAttr('style');
      }
      $("body").unbind("keyup");
      ...
      $(document).scrollTop(opts.documentScrollTop);
    }
	...
	function resizeWx (){
		var minheight = opts.height;
		var wHeight = $(window).height();
    var height = $("#Js-pop-body").height();
    if(wHeight>minheight){
      $("#wx-Js-pop-body-box").css("height",$("#body_box").height()+'px');
      $("#Js-pop-body").css({"margin-top":($(window).height()/2 +$(document).scrollTop() -height/2)});
    }else{
      $("#Js-pop-body").css({"margin-top":($(window).height()/2 +$(document).scrollTop() -height/2)});
    }
    ...
    if(opts.attachBg){
      ...
      if(opts.height == undefined || $(window).height()>opts.height) {
    	  var wHeight = $("#body_box").height()+80;
      }else{
    	  var wHeight = $("#body_box").height()+80;
      }
      
      $("#wx-Js-pop-body-box").css({"overflow":"hidden","position":"absolute",'top':'0','width':'100%',"height":wHeight});
      //滚动条禁用
      var top_so = $(document).scrollTop();
      $(document).on('scroll.unable',function(e){
        $(document).scrollTop(top_so);
      })
      _setEleToCenter("#Js-pop-body",opts,wHeight);
      
      $("#body_box").css({"overflow":"hidden","position":"relative","height":"auto"});
     // $("#Js-shadeLayer").css({"width":$(window).width(),"height":$(document).height()});
      $("#Js-shadeLayer").css({"width":'100%',"height":wHeight});
  ...
  function _popAni(id, callback, opts) {
    ...
    if(opts.notAni){
      ...
      o.css("opacity",0);
      o.stop().animate({"opacity":1,"top":top+30},100,$.isFunction(callback)?callback:undefined);
    }
  }
  ...
  function _closeAni(id, callback, opts) {
    ...
    $("body").parent().css("overflow-y","auto").css("overflow-x","auto");
    //滚动条启用
    $(document).unbind('scroll.unable');
  }

  //将元素设置为居中
  function _setEleToCenter(eleId, opts,wHeight) {
    opts = opts || {};
    ...
    $ele.css({"margin": 'auto' || (_winWidth/2-width/2+(opts.offsetX||0)),"margin-top" : opts.y || ($(window).height()/2 +$(document).scrollTop() -height/2)});
  }
  ...
  function _getShadeLayer(layerClass) {
   ...
   // return '<div id="Js-shadeLayer" class="'+layerClass+' pop-bg" style="width:'+_winWidth+'px;height:'+window_height+'px;"></div>';
    return '<div id="Js-shadeLayer" class="'+layerClass+' pop-bg" style="width:100%;height:'+window_height+'"></div>';
  }
...
if(typeof wx !== "undefined" && typeof jQuery !== "undefined"){
	  $.extend(wx.config, {
	    ...
	    */
	    ...
	    //弹出框alert结构
	    alert: '<div class="panel panel-default" style="position: relative; z-index: 1001; margin: 0 auto 0 auto;width:<%tip_width%>;min-width:460px;">\
			<div class="panel-heading">\
				<button aria-hidden="true" data-dismiss="modal" class="close Js-pop-close" title="点击×或者按Esc键关闭窗口" type="button">×</button>\
				<span class="head-title" style="width: 90%;display: inline-block;white-space: nowrap;overflow: hidden; vertical-align: middle;font-size:18px;color:#ffffff;"><%tip_title%></span>\
			</div>\
			<div style="padding: 48px 30px;text-align: left;<%style%>"><%content%></div>\
			<div class="Js_editBut clearfix" style="margin-top: 15px;">\
				<a class="button_alert Js-pop-close" href="javascript:;" style="font-size: 18px;;display: inline-block;width: 100%;height: 50px;line-height: 50px;;text-align: center;background: #888fa7;color: #ffffff;letter-spacing: 2px;"><%tip_yes%></a>\
			</div>\
		</div>',

	    //弹出框confirm结构  确定Js-confirm-ok
	    confirm: '<div class="panel panel-default" style="position: relative; z-index: 1001; margin: 0 auto 0 auto;width:<%tip_width%>;">\
			<div class="panel-heading">\
				<button aria-hidden="true" data-dismiss="modal" class="close Js-pop-close" title="点击×或者按Esc键关闭窗口" type="button">×</button>\
				<span class="head-title" style="width: 90%;display: inline-block;white-space: nowrap;overflow: hidden; vertical-align: middle;font-size:20px;color:#ffffff;"><%tip_title%></span>\
			</div>\
			<div style="min-height:60px;line-height: 26px;padding: 45px 20px 20px 20px;text-align: left;"><%tip_txt%></div>\
				<div class="Js_editBut clearfix" style="height:50px;background:#888fa7;">\
					<a class="keep_this AllChangeClick fl" id="Js-confirm-ok" ><%tip_yes%></a>\
					<a class="ques_this Js-pop-close fl" ><%tip_no%></a>\
				</div>\
			</div>'	
	  });
}
