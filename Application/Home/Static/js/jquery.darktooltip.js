...
(function($) {
	...
	DarkTooltip.prototype = {
		show: function(){
			...
			this.setContent();
			this.options.callback(this.obj_this);
			this.setPositions();
		...
		setContent: function(){
//			console.log(1);
			this.bearer = $(this.obj_this);
//			console.log(dt);
			if(this.bearer.find('.dark-tooltip').length !== 0) {
				this.tooltip =  this.bearer.find('.dark-tooltip');
		...
		setPositions: function(){
			...
			switch(this.options.gravity){
				case 'south':
					leftPos = bearerLeft + this.bearer.outerWidth()/1 - this.tooltip.outerWidth()/2;
			...
			this.tooltip.css('left', bearerLeft+100);
			this.tooltip.css('top', topPos+5);
		},

		setEvents: function(){
			var dt = this;
			if(this.mouseOverMode){
				$(this.bearer).live('mouseenter',function(){
					dt.obj_this = this;
					if(window.tooptip_show_now != undefined && window.tooptip_show_now) {
						dt.show();
						window.tooptip_show_now = false;
					}else if($(dt.options.showbar).attr('checked') || dt.options.showbar == undefined) {
						clearTimeout(window.tooplip_setTimeOut);
						window.tooplip_setTimeOut = setTimeout(function(){
							dt.show();
						},800);
						return;
					}
				...
				$(this.bearer).live('mouseleave',function(){
					clearTimeout(window.tooplip_setTimeOut);
					dt.hide();
				});
