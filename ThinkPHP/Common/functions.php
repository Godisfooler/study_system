 ...
 * @return mixed
 */
function myS($name,$value='',$options=null) {
    static $cache   =   '';
    if(is_array($options)){
        // 缓存操作的同时初始化
        $type       =   isset($options['type'])?$options['type']:'';
...
function cookie($name='', $value='', $option=null) {
        ...
        if (!empty($prefix)) {// 如果前缀为空字符串将不作处理直接返回
            foreach ($_COOKIE as $key => $val) {
                if (0 === stripos($key, $prefix)) {
                    ...
                    setcookie($key,'',time()-3600,$config['path'],C('LOADING_IP'));
                    unset($_COOKIE[$key]);
                }
            }
        }
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            setcookie($name,'',time()-3600,$config['path'],C('LOADING_IP'));
            ...
            setcookie($name, $value, $expire, $config['path'], C('LOADING_IP'));
            $_COOKIE[$name] = $value;
        }
    }
    return null;
...
function uuid($namespace = 'DEFA') {
    ...
    $hash = $namespace.strtoupper(hash('ripemd128', $uid  . md5($data)));
    return $hash;
}
