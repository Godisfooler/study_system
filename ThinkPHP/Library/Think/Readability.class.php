...
class Readability {
    ...
    private $junkTags = Array("style", "form", "iframe", "script", "button", "input", "textarea", 
                                ...
                                "embed", "frame", "frameset", "keygen", "label", "marquee", "link", "img");
    // 需要删除的属性
    ...
    private function getTopBox() {
        ...
        while($paragraph = $allParagraphs->item($i++)) {
            ...
            if (preg_match("/(comment|meta|footer|footnote)/i", $className)) {
                $contentScore -= 50;
            } else if(preg_match("/((^|\\s)(post|hentry|entry[-]?(content|text|body)?|article[-]?(content|text|body)?)(\\s|$))/i", $className)) {
                $contentScore += 25;
            }
            // Look for a special ID
            if (preg_match("/(comment|meta|footer|footnote)/i", $id)) {
                $contentScore -= 50;
            } else if (preg_match("/^(post|hentry|entry[-]?(content|text|body)?|article[-]?(content|text|body)?)$/i", $id)) {
                $contentScore += 25;
            }
    ...
    public function getTime(){
    	$dateNodes = $this->DOM->getElementsByTagName("em");
    	if ($dateNodes->length && $dateNode = $dateNodes->item(0)) {
    		// @see http://stackoverflow.com/questions/717328/how-to-explode-string-right-to-left
    ...
    public function getGzh(){
    	$spans_list = $this->DOM->getElementsByTagName('span');
    	$spans_count = $spans_list->length;
    	
    	$gzh = "";
    	foreach ($spans_list as $span){
    		$spanProp = $span->getAttribute('class');
    		if($spanProp == "rich_media_meta rich_media_meta_text rich_media_meta_nickname"){
    			$gzh = $span->nodeValue;
    			break;
    		}
    	}
    ...
    public function getContent($type = "") {
        ...
        if($type == 'weixin'){
        	//获取发布时间
        	$ContentTime = $this->getTime();
        	$ContentGzh = $this->getGzh();
        }
        ...
        return Array(
            ...
            'time' => $ContentTime ? $ContentTime : null,
            'gzh' => $ContentGzh ? $ContentGzh : null,
            'content' => $content
        );
    }
    function __destruct() { }
