...
class OssClient
{
     ...
     * @return string
     */
    public function getObject($bucket, $object, $options = NULL,$status=0 ,$ContentType=NULL,$ContentDisposition=NULL)
    {   
        if($ContentType!==NULL) $options[self::OSS_CONTENT_TYPE] = $ContentType;
        // if($ContentDisposition !==NULL) $options[self::OSS_CONTENT_DISPOSTION] = $ContentDisposition;
        $this->precheckCommon($bucket, $object, $options);
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_GET;
        $options[self::OSS_OBJECT] = $object;
        ...
        $response = $this->auth($options,$status);
     ...
     * @throws OssException
     */
    public function signUrl($bucket, $object, $timeout = 60, $status = 0, $method = self::OSS_HTTP_GET, $options = NULL)
    {   
        $this->precheckCommon($bucket, $object, $options);
        //method
        ...
        $re = $this->setSignStsInUrl(true);
//         var_dump($options,$this->auth($options,$status));
//         exit;
        return $this->auth($options,$status);
    }
     ...
     */
    private function auth($options,$status=0)
    {
        OssUtil::validateOptions($options);
    ...
    private function generateSignableQueryStringParam($options)
    {
        ...
        // if(isset($options['Content-Type'])) $signableQueryStringParams['response-content-type'] = $options['Content-Type'];
        // if(isset($options['Content-Disposition'])) $signableQueryStringParams['response-content-disposition'] = $options['Content-Disposition'];
     ...
     * @return bool
     */
    public function doesObjectExistContentSizeNotZero($bucket, $object,$status = 0, $options = NULL)
    {
        $this->precheckCommon($bucket, $object, $options);
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_HEAD;
        ...
        $response = $this->auth($options,$status);
        $result = new ExistResult($response);
        if($result->getData() && $result->getRawResponse()->header['content-length'] > 0) {
            return true;
        }else{
            return false;
        }
