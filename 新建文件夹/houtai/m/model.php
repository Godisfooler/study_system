...
function sel_m_table($table){
	return mysqlii_db("select * from $table");
}
/**
 * 标示通过条件查询数据
 * @param 表示表名 以数组方式传入
...
function db_query($sql){
	$DB_Conf=DB_C();
	$conn=mysqli_connect($DB_Conf['HOST'].":".$DB_Conf['Port'],$DB_Conf['DB_Name'],$DB_Conf['DB_Pwd']);
	mysqli_select_db($conn,$DB_Conf['DB_Database']);
	$res=mysqli_query($conn,$sql);
	if($res){
	return $re=mysqli_affected_rows($conn);
	}else{
	return '';
	}
}
function mysqli_db_cz($sql){
	$DB_Conf=DB_C();
	$conn=mysqli_connect($DB_Conf['HOST'].":".$DB_Conf['Port'],$DB_Conf['DB_Name'],$DB_Conf['DB_Pwd']);
	mysqli_select_db($conn,$DB_Conf['DB_Database']);
	mysqli_query($conn,"set names 'utf8'");//操作  写入 修改编码为utf8
	$res=mysqli_query($conn,$sql);
	return $re=mysqli_affected_rows($conn);//表示mysqli当前链接操作影响函数查询
}
function mysqli_db($sql){
	$rows=array();
	$es=array();
	$DB_Conf=DB_C();
	$conn=mysqli_connect($DB_Conf['HOST'].":".$DB_Conf['Port'],$DB_Conf['DB_Name'],$DB_Conf['DB_Pwd']);
	mysqli_select_db($conn,$DB_Conf['DB_Database']);
	mysqli_query($conn,"set character set 'utf8'");//读数据库 设置编码为UTF8格式
	$res=mysqli_query($conn,$sql);
