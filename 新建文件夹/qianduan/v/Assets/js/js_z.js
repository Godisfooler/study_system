$(function(){
	...
	$('.h_bg .bg_img img').css('margin-left','-'+w_banner+'px');
	$('.banner_s img').css('margin-left','-'+w_banner+'px');
	$('.i_mbm table tr:nth-child(2n)').children('td').css({'background':'#fff','height':'42px','line-height':'42px'});
	$('.partner ul li:nth-child(7n)').css('margin-right',0+'px');
	
	
	$('.join li.now .des').slideDown();
	$('.join li .li_m').click(function(){
		$(this).parents('li').siblings('li').removeClass('now');
		$(this).parents('li').siblings('li').children('.des').slideUp();
		$(this).parents('li').addClass('now');
		$(this).siblings('.des').slideDown();
	});
})
//左右点击滚动
